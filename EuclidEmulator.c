...
int main(int argc,char **argv) {
    ...
    int i,di,ip,iz,ic,j,l,lmax,cnt;
    double x,prod,z,t0,t200,dDelta,t,dti;
    ...
    size = s.st_size;
    /*
    ** Get cosmo params from the command line in order and 
    ** make sure they are in range.
    */
    //printf("Reading in command line parameters...\n");
    assert(argc == 8);
    for (ip=0;ip<6;++ip) {
    ...
    */
    ...
    for (ip=0;ip<6;++ip) {
      ...
      assert(Pl[ip] != NULL);
      x = 2*(cp[ip] - min[ip])/(max[ip]-min[ip]) - 1.0;
    ...
    */
    ...
    csm->val.dOmega0 = cp[1]/(cp[3]*cp[3]);
    csm->val.w0 = cp[4];
